...
pub(crate) fn for_current_location() -> Result<(String, f64, f64, String), Box<dyn std::error::Error>>  {
    // Uses determine_generic_query to ask user for location
    let full_location: String = crate::journal::query::for_string("What's your current location?", 
...
pub(crate) fn for_desired_datetime(timezone: &str) -> Result<chrono::DateTime<chrono_tz::Tz>, Box<dyn std::error::Error>> {
    let mut timezone = String::from(timezone);
    loop {
        let current_date = crate::journal::calculators::get_current_date_from_tz(&timezone) ?;
        ...
        match crate::journal::query::for_bool("Is this the timezone you want to use?") ? {
            ...
            false => { timezone = crate::journal::query::for_string("What should the timezone be?", "Enter an IANA Timezone ") ?}
        };
    }
}
...
pub(crate) fn for_location_from_list(results: &[crate::journal::Location]) -> Result<&crate::journal::Location, Box<dyn std::error::Error>> {
    if results.is_empty() {
        ...
        let choice: usize = crate::journal::query::for_usize("Enter the number of the city you're in")?;
